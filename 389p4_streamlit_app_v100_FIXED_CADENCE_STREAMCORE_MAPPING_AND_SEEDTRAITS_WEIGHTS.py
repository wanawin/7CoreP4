...
def _parse_cadence_table_from_text(text: str) -> pd.DataFrame | None:
    """Parse a cadence *table* from uploaded text.

    Supports:
      1) Legacy cadence tables with columns: level, core_family, stream, days_since_last_hit, ...
      2) Stream-only cadence exports (no level/core_family) with columns like:
         stream, hits_last180, days_since_last_hit, avg_gap_days, median_gap_days, ...

    Returns a normalized DataFrame with at least:
      - stream (original label)
      - stream_key (canonical key for matching)
      - DaysSinceLastHit (int/float)
      - mean_gap_days (float, optional)
      - CadenceScore (float)  [computed if possible; else NaN]
      - level/core_family are preserved if present; otherwise level='stream', core_family='*'
    """
    try:
        if not isinstance(text, str) or not text.strip():
            return None

        # Try CSV first (covers .csv as well as .txt that is CSV-formatted)
        ...
        if "stream" not in cols_l:
            return None

        # Normalize stream string + stream_key
        stream_col = cols_l["stream"]
        df[stream_col] = df[stream_col].astype(str).str.strip()
        ...
        df["stream"] = df[stream_col]
        df["stream_key"] = df["stream"].map(_canon_stream)

        # Detect legacy vs stream-only
        has_level = "level" in cols_l
        has_core = "core_family" in cols_l

        if has_level:
            df["level"] = df[cols_l["level"]].astype(str).str.strip().str.lower()
        else:
            df["level"] = "stream"

        if has_core:
            core_raw = df[cols_l["core_family"]].astype(str).str.strip()
            df["core_family"] = core_raw.map(lambda x: x.zfill(3) if str(x).isdigit() else str(x))
        else:
            df["core_family"] = "*"

        # Map numeric columns (accept multiple naming variants)
        # DaysSinceLastHit
        dslh_col = cols_l.get("days_since_last_hit") or cols_l.get("dayssincelasthit")
        if dslh_col:
            df["DaysSinceLastHit"] = pd.to_numeric(df[dslh_col], errors="coerce")
        else:
            df["DaysSinceLastHit"] = np.nan

        # mean_gap_days
        mg_col = cols_l.get("mean_gap_days") or cols_l.get("avg_gap_days") or cols_l.get("average_gap_days")
        if mg_col:
            df["mean_gap_days"] = pd.to_numeric(df[mg_col], errors="coerce")
        else:
            df["mean_gap_days"] = np.nan

        # Coerce optional numeric columns if present
        for src in ["median_gap_days","avg_gap_days","hits_last90","hits_last180","hits_last365","hits_all","hits_per_week_full","hits_per_week_recent"]:
            c = cols_l.get(src)
            if c:
                df[src] = pd.to_numeric(df[c], errors="coerce")

        # Precompute cadence score if we have enough information
        # Uses the app's cadence formula: higher when (DaysSinceLastHit is in a reasonable band around mean_gap_days),
        # but never negative.
        try:
            df["CadenceScore"] = df.apply(
                lambda r: compute_cadence_score(r.get("DaysSinceLastHit", np.nan), r.get("mean_gap_days", np.nan)),
                axis=1
            )
        except Exception:
            df["CadenceScore"] = np.nan
            ...
            if cadence_table_df is not None and not cadence_table_df.empty:
                try:
                    ...
                    def _ensure_cadence_cols(df: pd.DataFrame) -> pd.DataFrame:
                        """Normalize cadence export columns so merges + scoring work across variants."""
                        if df is None or df.empty:
                            return pd.DataFrame(columns=["Stream","Core","MeanGapDays","DaysSinceLastHit","CadenceScore"])

                        out = df.copy()

                        # --- standardize column names (case-insensitive, tolerate variants) ---
                        ...
                        # Stream / Core
                        c_stream = _pick("Stream", "stream")
                        c_core   = _pick("Core", "core", "CORE")

                        if c_stream and c_stream != "Stream":
                            out = out.rename(columns={c_stream: "Stream"})
                        if c_core and c_core != "Core":
                            out = out.rename(columns={c_core: "Core"})

                        # Days since last hit
                        c_dslh = _pick("DaysSinceLastHit", "days_since_last_hit", "dslh", "DaysSinceHit", "days_since_hit")
                        if c_dslh and c_dslh != "DaysSinceLastHit":
                            out = out.rename(columns={c_dslh: "DaysSinceLastHit"})

                        # Mean gap days
                        c_gap = _pick("MeanGapDays", "mean_gap_days", "avg_gap_days", "AverageGapDays", "AvgGapDays")
                        if c_gap and c_gap != "MeanGapDays":
                            out = out.rename(columns={c_gap: "MeanGapDays"})

                        # HitsPerWeek (optional)
                        c_hpw = _pick("HitsPerWeek", "hits_per_week", "HPW")
                        if c_hpw and c_hpw != "HitsPerWeek":
                            out = out.rename(columns={c_hpw: "HitsPerWeek"})

                        # Convert numeric
                        for c in ["DaysSinceLastHit", "MeanGapDays", "HitsPerWeek"]:
                            if c in out.columns:
                                out[c] = pd.to_numeric(out[c], errors="coerce")

                        # Map common cadence export headers -> canonical columns used by the app
                        # (Your cadence export uses lowercase headers like 'stream' and 'core_family'.)
                        colmap = {c.lower(): c for c in out.columns}
                        if "stream" in colmap and "Stream" not in out.columns:
                            out["Stream"] = out[colmap["stream"]].astype(str)
                        if "core_family" in colmap and "Core" not in out.columns:
                            out["Core"] = out[colmap["core_family"]].astype(str)
                        if "core" in colmap and "Core" not in out.columns:
                            out["Core"] = out[colmap["core"]].astype(str)
                        if "family" in colmap and "Core" not in out.columns:
                            out["Core"] = out[colmap["family"]].astype(str)
                        if "member" in colmap and "Member" not in out.columns:
                            out["Member"] = out[colmap["member"]]
                        # Ensure Stream/Core exist (for merge keys)
                        if "Stream" not in out.columns:
                            out["Stream"] = ""
                        if "Core" not in out.columns:
                            out["Core"] = ""

                        # Normalize Core to 3-digit text
                        out["Core"] = out["Core"].astype(str).str.strip().str.zfill(3)

                        # Build a reasonable Hits180 proxy if missing (used by some older scoring)
                        if "Hits180" not in out.columns:
                            if "HitsPerWeek" in out.columns:
                                out["Hits180"] = (out["HitsPerWeek"] * (180.0/7.0)).fillna(0.0)
                            else:
                                out["Hits180"] = 0.0

                        # Compute CadenceScore if absent or all-null
                        if ("CadenceScore" not in out.columns) or out["CadenceScore"].isna().all():
                            if ("DaysSinceLastHit" in out.columns) and ("MeanGapDays" in out.columns):
                                out["CadenceScore"] = out.apply(
                                    lambda r: compute_cadence_score(r.get("DaysSinceLastHit"), r.get("MeanGapDays")),
                                    axis=1
                                )
                            else:
                                # fallback (very conservative)
                                dslh = out.get("DaysSinceLastHit", pd.Series([np.nan]*len(out)))
                                hits180 = out.get("Hits180", pd.Series([0]*len(out)))
                                out["CadenceScore"] = ((hits180 > 0) & (dslh <= 180)).astype(float)

                        # Canonical stream key used for merges
                        out["_stream_key"] = out["Stream"].astype(str).apply(_canon_stream)
            ...
            if _enable_seed_traits_boost and (_pos_lookup is not None) and (_neg_lookup is not None):
                ...
                if "Seed" in ranked_lines.columns and ("Stream" in ranked_lines.columns):
                    ...
                    if _core_col is not None:
                        ranked_lines["SeedTraitsScore"] = ranked_lines.apply(
                            lambda r: compute_seed_traits_score(
                                seed=str(r.get("Seed","")).zfill(4),
                                core=str(r.get(_core_col,"")).zfill(3),
                                stream=str(r.get("Stream","")),
