...
with _t_nl:
                    ...
                    if st.session_state.get('enable_cadence', True):
                        ...
                        try:
                            if cadence_table_df is not None and not cadence_table_df.empty:
                                _cad = cadence_table_df
                                core_col = "core_family" if "core_family" in _cad.columns else ("Core" if "Core" in _cad.columns else ("core" if "core" in _cad.columns else None))
                                stream_col = "stream" if "stream" in _cad.columns else ("Stream" if "Stream" in _cad.columns else None)
                                if core_col and stream_col:
                                    _m = _cad[
                                        _cad[core_col].astype(str).str.strip().str.zfill(3).eq(str(core_key).strip().zfill(3))
                                        & _cad[stream_col].astype(str).str.strip().eq(str(stream).strip())
                                    ]
                                    # Prefer core-level rows if present
                                    if "level" in _m.columns:
                                        _m_core = _m[_m["level"].astype(str).str.strip().str.lower().eq("core")]
                                        if not _m_core.empty:
                                            _m = _m_core
                                    # Core-level member is blank/NaN in exports (accept '*' too for legacy)
                                    if "member" in _m.columns:
                                        _m_core2 = _m[_m["member"].isna() | (_m["member"].astype(str).str.strip().isin(["", "*", "nan", "none"]))]
                                        if not _m_core2.empty:
                                            _m = _m_core2
                                    if not _m.empty:
                                        avg_gap_days = float(_m.iloc[0].get('avg_gap_days') or 0.0)
                        except Exception:
                            pass
                        if not avg_gap_days or avg_gap_days <= 0:
                            h_stream = float(_hits_by_stream.get(str(stream), 0.0) or 0.0)
    ...
    with _t_ns:
        ...
        if stats_ns is not None and not stats_ns.empty:
            ...
            if "PctStrength" not in ns_df.columns and "PosPctStrength" in ns_df.columns:
                ns_df["PctStrength"] = pd.to_numeric(ns_df["PosPctStrength"], errors="coerce").fillna(0.0)
    ...
    with _t_exec:
        ...
        try:
            if (
                ...
                # Cadence: merge cadence score by (Stream, core) using a robust, canonical stream key.
                try:
                    if isinstance(globals().get("cadence_table_df", None), pd.DataFrame) and (not globals()["cadence_table_df"].empty):
                        ...
                        def _norm_stream_key(x):
                            x = "" if x is None else str(x)
                            x = x.replace("|", " | ")
                            x = re.sub(r"\s+", " ", x).strip().lower()
                            return x

                        ranked_lines["__STREAM_KEY__"] = ranked_lines.get("Stream", "").apply(_norm_stream_key)
                        _cad["__STREAM_KEY__"] = _cad.get("Stream", "").apply(_norm_stream_key)

                        core_col = None
                        for cand in ["core", "Core", "CORE"]:
                            if cand in _cad.columns:
                                core_col = cand
                                break
                        if core_col is None:
                            core_col = "Core"
                            _cad[core_col] = _cad.get(core_col, "")

                        _cad["__CORE__"] = _cad[core_col].astype(str).str.zfill(3)
                        ranked_lines["__CORE__"] = ranked_lines.get("core", "").astype(str).str.zfill(3)

                        avg_gap = pd.to_numeric(_cad.get("avg_gap_days"), errors="coerce")
                        dslh = pd.to_numeric(_cad.get("DaysSinceLastHit", _cad.get("days_since_last_hit")), errors="coerce")
                        denom = avg_gap.fillna(0.0).replace(0.0, 1.0)
                        _cad["__CADENCE_SCORE__"] = (dslh.fillna(0.0) / denom).clip(lower=0.0, upper=5.0)

                        _cad_small = _cad[["__STREAM_KEY__", "__CORE__", "__CADENCE_SCORE__"]]                            .dropna(subset=["__STREAM_KEY__", "__CORE__"])                            .drop_duplicates(subset=["__STREAM_KEY__", "__CORE__"])

                        ranked_lines = ranked_lines.merge(
                            _cad_small,
                            how="left",
                            left_on=["__STREAM_KEY__", "__CORE__"],
                            right_on=["__STREAM_KEY__", "__CORE__"],
                        )

                        ranked_lines["CadenceScore"] = pd.to_numeric(ranked_lines.get("__CADENCE_SCORE__"), errors="coerce").fillna(0.0)
                        ranked_lines = ranked_lines.drop(columns=[c for c in ["__CADENCE_SCORE__"] if c in ranked_lines.columns])
                    else:
        ...
        # 1) PctStrength (position strength): ensure non-zero by building per-core RankPos -> PctStrength maps.
        #    This is debug-only and does NOT change ranking.
        try:
            need_ps = ("PctStrength" not in ranked_lines.columns)
            if (not need_ps) and ("PctStrength" in ranked_lines.columns):
                _ps = pd.to_numeric(ranked_lines["PctStrength"], errors="coerce").fillna(0.0)
                need_ps = bool(_ps.eq(0.0).all())

            if need_ps and ("RankPos" in ranked_lines.columns) and ("core" in ranked_lines.columns):
                base_df = st.session_state.get("ns_stream_rank_df")
                core_pos_map_cache = st.session_state.get("core_pos_map_cache", {}) or {}

                if base_df is not None and isinstance(base_df, pd.DataFrame) and ("core" in base_df.columns) and ("RankPos" in base_df.columns):
                    hit_col = "HitsWindow" if "HitsWindow" in base_df.columns else ("HitCount" if "HitCount" in base_df.columns else None)
                    if hit_col is not None:
                        for c in ranked_lines["core"].astype(str).str.zfill(3).unique():
                            if c in core_pos_map_cache:
                                continue
                            try:
                                src = base_df[base_df["core"].astype(str).str.zfill(3) == c][["RankPos", hit_col]].copy()
                                if len(src) == 0:
                                    continue
                                src = src.rename(columns={hit_col: "HitCount"})
                                pm = position_percentile_map(src)
                                core_pos_map_cache[c] = pm
                            except Exception:
                                continue

                st.session_state["core_pos_map_cache"] = core_pos_map_cache

                def _lookup_pct(r):
                    try:
                        c = str(r.get("core", "")).zfill(3)
                        rp = int(r.get("RankPos", 0) or 0)
                    except Exception:
                        return 0.0

                    pm = core_pos_map_cache.get(c)
                    if pm is None:
                        return 0.0
                    if isinstance(pm, dict):
                        return float(pm.get(rp, 0.0) or 0.0)
                    try:
                        mm = pm.loc[pm["RankPos"].astype(int) == rp, "PctStrength"]
                        if len(mm):
                            return float(mm.iloc[0])
                    except Exception:
                        pass
                    return 0.0

                ranked_lines["PctStrength"] = ranked_lines.apply(_lookup_pct, axis=1)
        except Exception:
            if "PctStrength" not in ranked_lines.columns:
                ranked_lines["PctStrength"] = 0.0

                # 2) SeedTraitsScore: compute per (stream, core) using the same lookups as the Traits audit: compute per (stream, core) using the same lookups as the Traits audit
        ...
        if ranked_lines is not None and not ranked_lines.empty:
            ...
            with st.expander("Score breakdown (7-core execution) — explain UniversalScore", expanded=False):
                # --- Ensure SeedTraitsScore + UniversalScore exist for breakdown (debug-only; no ranking changes) ---
                try:
                    # SeedTraitsScore
                    if "SeedTraitsScore" not in ranked_lines.columns:
                        if "SeedTraitScore" in ranked_lines.columns:
                            ranked_lines["SeedTraitsScore"] = pd.to_numeric(ranked_lines["SeedTraitScore"], errors="coerce").fillna(0.0)
                        elif "seed_traits_score" in ranked_lines.columns:
                            ranked_lines["SeedTraitsScore"] = pd.to_numeric(ranked_lines["seed_traits_score"], errors="coerce").fillna(0.0)
                        else:
                            ranked_lines["SeedTraitsScore"] = 0.0
                    # UniversalScore (recompute if missing)
                    if "UniversalScore" not in ranked_lines.columns:
                        for _c in ["BaseTerm", "DueTerm", "PosTerm", "SeedTerm", "CadTerm"]:
                            if _c not in ranked_lines.columns:
                                ranked_lines[_c] = 0.0
                        ranked_lines["UniversalScore"] = (
                            pd.to_numeric(ranked_lines["BaseTerm"], errors="coerce").fillna(0.0)
                            + pd.to_numeric(ranked_lines["DueTerm"], errors="coerce").fillna(0.0)
                            + pd.to_numeric(ranked_lines["PosTerm"], errors="coerce").fillna(0.0)
                            + pd.to_numeric(ranked_lines["SeedTerm"], errors="coerce").fillna(0.0)
                            + pd.to_numeric(ranked_lines["CadTerm"], errors="coerce").fillna(0.0)
                        )
                except Exception:
                    pass
