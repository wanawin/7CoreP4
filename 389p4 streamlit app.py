...
APP_VERSION = "v160_RESTORE_VALIDATION_AND_BACKTEST_CHARTS"
...
# ---------- Upload persistence helpers (prevents "lost upload" across reruns) ----------
class _NamedBytesIO(io.BytesIO):
    def __init__(self, b: bytes, name: str):
        super().__init__(b)
        self.name = name

def _persisted_uploader(label: str, key: str, types):
    """Streamlit resets uploaded files on app restarts and on some reruns.
    We persist bytes in session_state so the app can continue without forcing re-upload
    for every minor interaction. (Server redeploy/restart will still require re-upload.)
    """
    up = st.file_uploader(label, type=types, key=key)
    if up is not None:
        try:
            st.session_state[key + "_bytes"] = up.getvalue()
        except Exception:
            # Fallback for some UploadedFile variants
            st.session_state[key + "_bytes"] = up.read()
        st.session_state[key + "_name"] = getattr(up, "name", "upload")
    b = st.session_state.get(key + "_bytes", None)
    n = st.session_state.get(key + "_name", None)
    if b is None:
        return None
    return _NamedBytesIO(b, n or "upload")
# --- Safety defaults to prevent NameError across Streamlit reruns/tabs ---
# These are intentionally benign; later UI controls overwrite them when present.
cores_sel = []
all_dates_sorted = []
max_dates = None
only_hit_days = False
...
# --- BUILD STAMP (auto) ---
APP_BUILD_ID = "v159_FIX_IO_IMPORT_AND_BUILDSTAMP"
APP_BUILD_BASE = "base=v134_TRAITS_CACHE_SIG_FIX_POSMAPFIX_24HOPTIONAL"


def _download_filename(prefix: str, *, start_dt=None, end_dt=None, extra: str = "", ext: str = "csv") -> str:
    """Create unique, informative download names to avoid overwriting files."""
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    parts = [prefix]
    try:
        if start_dt is not None:
            parts.append(pd.to_datetime(start_dt).strftime("%Y-%m-%d"))
        if end_dt is not None:
            parts.append(pd.to_datetime(end_dt).strftime("%Y-%m-%d"))
    except Exception:
        pass
    if extra:
        safe = re.sub(r"[^A-Za-z0-9._-]+", "-", str(extra)).strip("-")
        if safe:
            parts.append(safe)
    parts.append(APP_BUILD_ID)
    parts.append(ts)
...
# --- Safe download filename helpers (prevents NameError/KeyError crashes) ---
def _safe_str(x, fallback: str = "na") -> str:
    try:
        if x is None:
            return fallback
        s = str(x)
...
def _safe_fname(prefix: str, ext: str = "csv") -> str:
    'Create stable filenames for download buttons without crashing if context vars are missing.'
    try:
        from datetime import datetime
        ts = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    except Exception:
        ts = "ts"
    p = _safe_str(prefix, "download")
    return f"{APP_BUILD_ID}__{p}__{ts}.{ext}"
# --------------------------
    ...
    if enable_member_seed_traits_overrides and (member_traits_pos_lut or member_traits_neg_lut):
        try:
            ...
            top1, top2 = _apply_member_traits_soft_override(
                ...
                df_all=df_all,
                member_pos_lut=(member_traits_pos_lut_auto if bool(st.session_state.get("auto_traits_mode", False)) else (member_traits_pos_lut or {})),
                member_neg_lut=(member_traits_neg_lut_auto if bool(st.session_state.get("auto_traits_mode", False)) else (member_traits_neg_lut or {})),
                weight=float(member_seed_traits_weight or 0.25),
                auto_mode=bool(st.session_state.get("auto_traits_mode", False)),
                auto_k=float(st.session_state.get("auto_traits_k", 250.0)),
                auto_cap=float(st.session_state.get("auto_traits_cap", 2.0)),
            )
...
# -------------------------
# AUTO-WEIGHTED TRAITS (per core/member) — additive evidence using lift + support
# -------------------------

def _shrink_factor(n: float, k: float = 250.0) -> float:
    """Sample-size shrinkage in [0,1). Larger n => closer to 1.0."""
    try:
        n = float(n)
    except Exception:
        return 0.0
...
def _safe_log_lift(lift: float) -> float:
    """Log-lift with guards. lift<=0 => 0.0; lift==1 => 0.0."""
    try:
        lift = float(lift)
    except Exception:
        return 0.0
    if not np.isfinite(lift) or lift <= 0:
        return 0.0
    return float(math.log(lift))

def _evidence_from_lift(lift: float, n: float, *, k: float = 250.0, cap_abs: float = 2.0) -> float:
    """Convert (lift,n) into a bounded evidence score (additive)."""
    ev = _safe_log_lift(lift) * _shrink_factor(n, k=k)
    if ev > cap_abs:
...
def _build_traits_lookup_with_support(df: pd.DataFrame) -> Dict[str, Dict[str, Dict[str, Tuple[float, float]]]]:
    """lookup[core_family][trait][value] -> (lift, n_support)

    Uses trait_total when available; otherwise falls back to trait_hits; else 0.
    """
    ...
    val_col = cols.get("value", None)
    lift_col = cols.get("lift", None)
    total_col = cols.get("trait_total", None)
    hits_col = cols.get("trait_hits", None)
    if not (core_col and trait_col and val_col and lift_col):
        return lookup
    for _, r in df.iterrows():
        try:
        ...
        n = 0.0
        try:
            if total_col and total_col in r and pd.notna(r[total_col]):
                n = float(r[total_col])
            elif hits_col and hits_col in r and pd.notna(r[hits_col]):
                n = float(r[hits_col])
        except Exception:
            n = 0.0
        lookup.setdefault(core, {}).setdefault(trait, {})[val] = (float(lift), float(n))
    return lookup

def _build_member_traits_lookup_with_support(df: pd.DataFrame) -> Dict[str, Dict[str, Dict[str, Dict[str, Tuple[float, float]]]]]:
    """lookup[core_family][member_label][trait][value] -> (lift, n_support)

    Uses trait_member_hits when available; else trait_core_hits; else 0.
    """
    lookup: Dict[str, Dict[str, Dict[str, Dict[str, Tuple[float, float]]]]] = {}
    if df is None or df.empty:
        return lookup
    cols = {c.lower(): c for c in df.columns}
    core_col = cols.get("core_family", None)
    ...
    val_col = cols.get("value", None)
    lift_col = cols.get("lift", None)
    mh_col = cols.get("trait_member_hits", None)
    ch_col = cols.get("trait_core_hits", None)
    if not (core_col and member_col and trait_col and val_col and lift_col):
        return lookup
    for _, r in df.iterrows():
        try:
            ...
            n = 0.0
            try:
                if mh_col and mh_col in r and pd.notna(r[mh_col]):
                    n = float(r[mh_col])
                elif ch_col and ch_col in r and pd.notna(r[ch_col]):
                    n = float(r[ch_col])
            except Exception:
                n = 0.0
            lookup.setdefault(core, {}).setdefault(mem_label, {}).setdefault(trait, {})[value] = (float(lift), float(n))
        except Exception:
            continue
    return lookup

def compute_seed_traits_score_auto(
    core: str,
    seed: Optional[str],
    stream: Optional[str],
    *,
    pos_lookup: Dict[str, Dict[str, Dict[str, Tuple[float, float]]]],
    neg_lookup: Dict[str, Dict[str, Dict[str, Tuple[float, float]]]],
    last5_union_digits_by_stream: Optional[Dict[str, set]] = None,
    cap: float = 2.0,
    k: float = 250.0,
) -> Tuple[float, List[Tuple[str, str, float, float, str, float]]]:
    """Auto-weighted variant: additive evidence using (lift,n) with shrinkage.

    Returns (net_score, matches) where matches include (trait,val,lift,n,sign,ev).
    """
    if seed is None:
        return 0.0, []
    core_key = canonical_core_key(core)
    last5_union = None
    ...
    feats = _feature_values_for_seed(str(seed), core_key, last5_union_digits=last5_union)

    matches: List[Tuple[str, str, float, float, str, float]] = []
    score = 0.0
    for trait, vals in feats.items():
        for val in vals:
            tup = pos_lookup.get(core_key, {}).get(trait, {}).get(val)
            if tup is not None:
                lift, n = tup
                ev = _evidence_from_lift(lift, n, k=k, cap_abs=cap)
                score += ev
                matches.append((trait, val, float(lift), float(n), "+", float(ev)))
            tupn = neg_lookup.get(core_key, {}).get(trait, {}).get(val)
            if tupn is not None:
                lift, n = tupn
                ev = _evidence_from_lift(lift, n, k=k, cap_abs=cap)
                score -= ev
...
def _apply_member_traits_soft_override(
    ...
    member_neg_lut: Dict[str, Dict[str, Dict[str, Dict[str, Any]]]],
    weight: float,
    ...
    def _lift_to_mult(x: Any) -> float:
        if auto_mode:
            try:
                if isinstance(x, (tuple, list)) and len(x) >= 2:
                    lift, n = float(x[0]), float(x[1])
                else:
                    lift, n = float(x), 0.0
            except Exception:
                lift, n = 1.0, 0.0
            ev = _evidence_from_lift(lift, n, k=float(auto_k), cap_abs=float(auto_cap))
            # Convert additive evidence back into a multiplicative factor
            try:
                return float(math.exp(ev))
            except Exception:
                return 1.0
        # legacy
        try:
    ...
    for lbl, p in base_probs.items():
        ...
        try:
            mult = float(mult) ** float(weight) if not auto_mode else float(mult) ** float(max(0.0, min(1.0, weight)))
        except Exception:
            mult = 1.0
        adj[lbl] = p * mult
...
def _explain_member_traits_soft_override(
    core: str,
    seed_digits: set,
    base_probs: dict,
    member_traits_pos_lut: dict,
    member_traits_neg_lut: dict,
    member_seed_weight: float,
    auto_mode: bool,
    auto_k: float,
    auto_cap: float,
    max_total_boost: float = 0.35,
):
    """Return detailed breakdown of member trait contributions for one (core, seed, stream) context.

    This mirrors _apply_member_traits_soft_override logic, but returns a detail table so you can verify:
    - which trait rows matched (pos/neg)
    - what multiplier they contributed to each member label
    - the blended multiplier and final adjusted probabilities
    """
    core = str(core).zfill(3)
    labels = ["AABC", "ABBC", "ABCC"]
    # start from base
    p0 = {k: float(base_probs.get(k, 0.0)) for k in labels}
    # multipliers accumulate in log-space
    log_mult = {k: 0.0 for k in labels}
    detail_rows = []

    def _row_fields(tr):
        # normalize expected fields; tolerate missing
        return {
            "trait": tr.get("trait", tr.get("Trait", tr.get("name", ""))),
            "seed_digit": tr.get("seed_digit", tr.get("SeedDigit", "")),
            "member_label": tr.get("member_label", tr.get("MemberLabel", tr.get("label", ""))),
            "lift": float(tr.get("lift", tr.get("Lift", 1.0)) or 1.0),
            "support": float(tr.get("support", tr.get("Support", tr.get("n", 0))) or 0.0),
            "direction": tr.get("direction", tr.get("Direction", "")),
        }

    for sd in sorted(seed_digits):
        pos_key = (core, str(sd))
        neg_key = (core, str(sd))
        pos_list = member_traits_pos_lut.get(pos_key, []) if member_traits_pos_lut else []
        neg_list = member_traits_neg_lut.get(neg_key, []) if member_traits_neg_lut else []

        # POS rules
        for tr in pos_list:
            f = _row_fields(tr)
            lbl = f["member_label"]
            if lbl not in log_mult:
                continue
            lift = max(1e-9, f["lift"])
            support = max(0.0, f["support"])
            # evidence shaping
            ev = min(support, float(auto_cap)) if auto_mode else support
            scale = 1.0
            if auto_mode:
                scale = float(ev) / (float(ev) + float(auto_k)) if (ev + auto_k) > 0 else 0.0
            # apply as a bounded log-mult (pos means lift>=1 is helpful)
            contrib = member_seed_weight * scale * float(np.log(lift))
            log_mult[lbl] += contrib
            detail_rows.append({
                "Core": core,
                "SeedDigit": str(sd),
                "Dir": "POS",
        ...
        for tr in neg_list:
            ...
            if auto_mode:
                scale = float(ev) / (float(ev) + float(auto_k)) if (ev + auto_k) > 0 else 0.0
            # NEG means penalize if lift>1 (or apply inverse if <1)
            contrib = member_seed_weight * scale * float(-np.log(lift))
            log_mult[lbl] += contrib
            detail_rows.append({
                "Core": core,
    ...
    # convert to multipliers, then blend (soft)
    mult_raw = {k: float(np.exp(v)) for k, v in log_mult.items()}
    mult_blend = {k: float(1.0 + max_total_boost * (mult_raw[k] - 1.0)) for k in labels}
    p1_unnorm = {k: p0[k] * mult_blend[k] for k in labels}
    s = sum(p1_unnorm.values())
    p1 = {k: (p1_unnorm[k] / s if s > 0 else p0[k]) for k in labels}

    detail_df = pd.DataFrame(detail_rows)
    # add computed multipliers to each row for convenience
    if not detail_df.empty:
        detail_df["MultRaw"] = detail_df["MemberLabel"].map(mult_raw)
        detail_df["MultBlend"] = detail_df["MemberLabel"].map(mult_blend)
...
def render_validation(df_all=None):
    """Diagnostics / validation view (no behavior changes)."""
    st.subheader("Validation")
    st.caption("Sanity checks for uploads + required columns. Useful when a deploy resets session state.")

    ss = st.session_state
    checks = [
        ("All-states history", "all_states_history"),
        ("24h map (optional)", "map24h_file"),
        ("Cadence file (optional)", "cadence_file"),
        ("Member seed-traits", "traits_member_seed"),
        ("Core seed-traits POS", "traits_core_pos"),
        ("Core seed-traits NEG", "traits_core_neg"),
        ("Seed→Winner CORE rules", "seed_rules_core"),
        ("Seed→Winner MEMBER rules", "seed_rules_member"),
    ]
        ...
        if ok:
            try:
                if isinstance(v,(bytes,bytearray)):
                    size=len(v)
                elif hasattr(v,'getbuffer'):
                    size=len(v.getbuffer())
            except Exception:
                size=None
        rows.append({"Item":label,"Key":key,"Loaded":ok,"Bytes":size})

    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

    if df_all is None or getattr(df_all,'empty',True):
        st.info("History dataframe is not loaded in this session yet. Re-upload the all-states file (left) then rebuild cache.")
        return

    st.markdown("#### History dataframe")
    st.write(f"Rows: {len(df_all):,} | Columns: {len(df_all.columns)}")

    cols=set(map(str, df_all.columns))
    required_groups=[
        ("Date", ["Date","date","DRAW_DATE"]),
        ("Result", ["Result","result","WinningNumbers","winning_numbers"]),
        ("State", ["State","state"]),
        ("Stream", ["Stream","stream"]),
    ]
    req_rows=[]
    for name,opts in required_groups:
        found=next((c for c in opts if c in cols), None)
        req_rows.append({"Field":name,"Found":bool(found),"Using":found or "—"})
...
"TrainCnt_ABCC": 0,
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name=_download_filename("backtest_core_stream_breakdown", start_dt=start_dt, end_dt=end_dt),
        mime="text/csv",
...
with st.sidebar:
    ...
    st.markdown("**Seed→Winner Digit Inclusion rules (wraparound) — optional**")
    seed_inclusion_core_file = st.file_uploader("Seed→Winner digit inclusion CORE rules CSV (optional)", type=["csv"], key="seed_inclusion_core_file")
    seed_inclusion_member_file = st.file_uploader("Seed→Winner digit inclusion MEMBER rules CSV (optional)", type=["csv"], key="seed_inclusion_member_file")

    st.divider()
    st.subheader("Auto-weighted Traits (per core/member) — optional decisioning")
    auto_traits_mode = st.checkbox("Enable AUTO-weighted traits (uses lift + support; reduces manual tuning)", value=True, key="auto_traits_mode")
    auto_traits_k = st.slider("AUTO shrinkage K (higher = more conservative on small samples)", 25.0, 2000.0, 250.0, 25.0, key="auto_traits_k")
    auto_traits_cap = st.slider("AUTO evidence cap (per trait match)", 0.25, 5.0, 2.0, 0.25, key="auto_traits_cap")
    st.caption("When AUTO mode is ON, seed/member trait contributions use log(lift) with sample-size shrinkage. Existing sliders still apply but should require far less tweaking.")
...
# Auto-weighted versions keep support counts when available
seed_traits_pos_lookup_auto = _build_traits_lookup_with_support(seed_traits_pos_df)
seed_traits_neg_lookup_auto = _build_traits_lookup_with_support(seed_traits_neg_df)
seed_traits_pos_lookup_auto = {**seed_traits_pos_lookup_auto, **(_build_traits_lookup_with_support(core_extra_pos_df) or {})}
seed_traits_neg_lookup_auto = {**seed_traits_neg_lookup_auto, **(_build_traits_lookup_with_support(core_extra_neg_df) or {})}


# Optional: incorporate Seed→Winner digit inclusion rules as additional seed-traits
def _seed_inclusion_rules_to_traits(df_rules: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Convert inclusion-rules CSV (core/member) into (pos_df, neg_df) in trait-table format."""
    if df_rules is None or df_rules.empty:
        return pd.DataFrame(), pd.DataFrame()
    df = df_rules.copy()
    # normalize columns
    cols = {c.lower(): c for c in df.columns}
    core_col = cols.get("core", cols.get("core_family"))
    cond_col = cols.get("condition")
    lift_col = cols.get("lift")
    n_col = cols.get("n")
    hits_col = cols.get("hits")
    if not (core_col and cond_col and lift_col):
        return pd.DataFrame(), pd.DataFrame()
    def _split_condition(s: str) -> Tuple[str, str]:
        s = str(s)
        if "==" in s:
            a, b = s.split("==", 1)
        ...
        if "=" in s:
            ...
            return a.strip(), b.strip()
        return s.strip(), "1"
    trait_vals = df[cond_col].apply(_split_condition)
    df["trait"] = trait_vals.apply(lambda x: x[0])
    df["value"] = trait_vals.apply(lambda x: x[1])
    df["core_family"] = df[core_col].astype(str).str.strip().str.zfill(3)
    df["lift"] = pd.to_numeric(df[lift_col], errors="coerce").fillna(1.0)
    df["trait_total"] = pd.to_numeric(df[n_col], errors="coerce").fillna(0) if n_col else 0
    df["trait_hits"] = pd.to_numeric(df[hits_col], errors="coerce").fillna(0) if hits_col else 0
    pos = df[df["lift"] > 1.0].copy()
    neg = df[df["lift"] < 1.0].copy()
    # For neg, store lift as-is (<1), handled by compute_seed_traits_score and auto the same way.
    return pos[["core_family","trait","value","lift","trait_total","trait_hits"]], neg[["core_family","trait","value","lift","trait_total","trait_hits"]]

try:
    _inc_pos, _inc_neg = _seed_inclusion_rules_to_traits(seed_inclusion_core_df)
    if not _inc_pos.empty:
        _inc_pos_lut = _build_traits_lookup(_inc_pos)
        seed_traits_pos_lookup = {**seed_traits_pos_lookup, **(_inc_pos_lut or {})}
        _inc_pos_lut_auto = _build_traits_lookup_with_support(_inc_pos)
        seed_traits_pos_lookup_auto = {**seed_traits_pos_lookup_auto, **(_inc_pos_lut_auto or {})}
    if not _inc_neg.empty:
        _inc_neg_lut = _build_traits_lookup(_inc_neg)
        seed_traits_neg_lookup = {**seed_traits_neg_lookup, **(_inc_neg_lut or {})}
        _inc_neg_lut_auto = _build_traits_lookup_with_support(_inc_neg)
        seed_traits_neg_lookup_auto = {**seed_traits_neg_lookup_auto, **(_inc_neg_lut_auto or {})}
except Exception:
    pass
...
member_traits_pos_lut_auto = _build_member_traits_lookup_with_support(member_traits_pos_df)
member_traits_neg_lut_auto = _build_member_traits_lookup_with_support(member_traits_neg_df)
...
# Seed→Winner digit inclusion (wraparound) rules (optional)
seed_inclusion_core_df = _read_local_or_uploaded_csv(globals().get("seed_inclusion_core_file", None), DEFAULT_SEED_RULES_CORE_PATH if "DEFAULT_SEED_RULES_CORE_PATH" in globals() else None)
seed_inclusion_member_df = _read_local_or_uploaded_csv(globals().get("seed_inclusion_member_file", None), DEFAULT_SEED_RULES_MEMBER_PATH if "DEFAULT_SEED_RULES_MEMBER_PATH" in globals() else None)
# Normalize to empty if None
if seed_inclusion_core_df is None:
    seed_inclusion_core_df = pd.DataFrame()
if seed_inclusion_member_df is None:
    seed_inclusion_member_df = pd.DataFrame()
...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    ...
    if use_all:
        ...
        st.session_state["bt_cores_sel_last"] = list(cores_sel)
    ...
    # Default to last 365 days (or full range if shorter)
    default_start = max(dmin.date(), (dmax - pd.Timedelta(days=365)).date())
    ...
    # Manual typing override (YYYY-MM-DD). Leave blank to use calendar values.
    c1, c2 = st.columns(2)
    with c1:
        start_date_text = st.text_input("Start date (type to override)", value="", key="bt_start_date_text", help="Optional. Format: YYYY-MM-DD")
    with c2:
        end_date_text = st.text_input("End date (type to override)", value="", key="bt_end_date_text", help="Optional. Format: YYYY-MM-DD")

    def _parse_manual_date(s: str) -> Optional[pd.Timestamp]:
        try:
            s = str(s).strip()
            if not s:
                return None
    ...
    start_dt = _parse_manual_date(start_date_text) or pd.Timestamp(start_date)
    end_dt = _parse_manual_date(end_date_text) or pd.Timestamp(end_date)

    # Clamp to available history range
    start_dt = max(start_dt, pd.Timestamp(dmin.date()))
    end_dt = min(end_dt, pd.Timestamp(dmax.date()))

    # NOTE: this value is referenced later in the tab; always re-pull from session_state
    # to avoid NameError if Streamlit re-runs mid-block.
    st.session_state.setdefault("bt_only_hit_days", True)
    only_hit_days = st.checkbox(
        "Diagnostic only: evaluate only days where a selected core member hit (faster) — NOT valid for EV",
        value=bool(st.session_state.get("bt_only_hit_days", True)),
        key="bt_only_hit_days",
    )
    only_hit_days = bool(st.session_state.get("bt_only_hit_days", True))
    ...
    if not use_cached:
        ...
        if bool(st.session_state.get("bt_only_hit_days", True)):
            candidate_dates = []
        ...
        st.session_state["wf_backtest_df_all"] = df_all.copy()
    ...
    try:
        st.caption("Overall predicted vs missed")
        _overall = pd.DataFrame({
            "Outcome": ["Predicted", "Missed"],
            "Count": [int(out["Predicted"].sum()), int((~out["Predicted"]).sum())],
        }).set_index("Outcome")
        st.bar_chart(_overall)
    except Exception:
        pass


    # --- Additional backtest charts (restored) ---
    try:
        if isinstance(kpi_df, pd.DataFrame) and not kpi_df.empty:
            # Ensure expected numeric columns
            for _c in ["BoxesPlayed", "BoxHit", "StraightsPlayed", "StraightHit"]:
                if _c in kpi_df.columns:
                    kpi_df[_c] = pd.to_numeric(kpi_df[_c], errors="coerce").fillna(0).astype(int)

            # 1) Hit rates by stream (Box)
            st.subheader("Box hit rate by stream")
            _by_stream = (
                kpi_df.groupby("Stream", dropna=False)[["BoxHit", "BoxesPlayed"]]
                .sum()
                .reset_index()
            )
            _by_stream["BoxHitRate"] = _by_stream.apply(
                lambda r: (r["BoxHit"] / r["BoxesPlayed"]) if r["BoxesPlayed"] else 0.0,
                axis=1,
            )
            _by_stream = _by_stream.sort_values("BoxHitRate", ascending=False).head(25)
            st.bar_chart(_by_stream.set_index("Stream")[["BoxHitRate"]])

            # 2) Hit rates by stream (Straight) – only if any straight tickets were played
            if int(kpi_df.get("StraightsPlayed", pd.Series([0])).sum()) > 0:
                st.subheader("Straight hit rate by stream")
                _by_stream_s = (
                    kpi_df.groupby("Stream", dropna=False)[["StraightHit", "StraightsPlayed"]]
                    .sum()
                    .reset_index()
                )
                _by_stream_s["StraightHitRate"] = _by_stream_s.apply(
                    lambda r: (r["StraightHit"] / r["StraightsPlayed"]) if r["StraightsPlayed"] else 0.0,
                    axis=1,
                )
                _by_stream_s = _by_stream_s.sort_values("StraightHitRate", ascending=False).head(25)
                st.bar_chart(_by_stream_s.set_index("Stream")[["StraightHitRate"]])

            # 3) Monthly hit rate trend (Box)
            st.subheader("Box hit rate over time (monthly)")
            _tmp = kpi_df.copy()
            if "Date" in _tmp.columns:
                _tmp["Date"] = pd.to_datetime(_tmp["Date"], errors="coerce")
                _tmp = _tmp.dropna(subset=["Date"])
                if not _tmp.empty:
                    _tmp["Month"] = _tmp["Date"].dt.to_period("M").astype(str)
                    _m = _tmp.groupby("Month")[["BoxHit", "BoxesPlayed"]].sum().reset_index()
                    _m["BoxHitRate"] = _m.apply(
                        lambda r: (r["BoxHit"] / r["BoxesPlayed"]) if r["BoxesPlayed"] else 0.0,
                        axis=1,
                    )
                    st.line_chart(_m.set_index("Month")[["BoxHitRate"]])

            # 4) Tickets played distribution (Box & Straight)
            st.subheader("Tickets played (total)")
            _t = pd.DataFrame(
                {
                    "BoxesPlayed": [int(kpi_df["BoxesPlayed"].sum())] if "BoxesPlayed" in kpi_df.columns else [0],
                    "StraightsPlayed": [int(kpi_df["StraightsPlayed"].sum())] if "StraightsPlayed" in kpi_df.columns else [0],
                }
            )
            st.bar_chart(_t.T)

            # 5) Seed frequency (Top 25) – helps verify seeds are flowing through
            if "Seed" in kpi_df.columns:
                st.subheader("Top seeds in evaluated rows")
                _seed_counts = (
                    kpi_df["Seed"]
                    .astype(str)
                    .replace({"None": "", "nan": ""})
                ...
                if not _seed_counts.empty:
                    st.bar_chart(_seed_counts)
                else:
                    st.info("No seeds available in KPI rows (this would indicate seed extraction is not flowing).")
    except Exception:
        pass
...
with st.expander("Traits verification panel (member seed-traits) — verify matches + contributions", expanded=False):
    if "wf_backtest_out" not in st.session_state or "wf_backtest_df_all" not in st.session_state:
        st.info("Run a backtest first — this panel uses the last walk-forward run stored in memory.")
    else:
        _out_dbg = st.session_state["wf_backtest_out"].copy()
        _df_all_dbg = st.session_state["wf_backtest_df_all"].copy()

        # pick a row from the most recent run
        _max_i = max(0, len(_out_dbg) - 1)
        _i = int(st.number_input("Pick a backtest row index to inspect", min_value=0, max_value=_max_i, value=min(0, _max_i), step=1))
        r = _out_dbg.iloc[_i].to_dict()

        st.markdown("**Row context**")
        _ctx_cols = ["Date", "Stream", "Winner", "Core", "ActualMemberLabel", "PredTop1Label", "PredTop2Label", "MemberTrainN"]
        _ctx = {k: r.get(k, "") for k in _ctx_cols if k in r}
        if not _ctx:
            # fallback if column names differ
            _ctx = {k: r.get(k, "") for k in ["Date", "Stream", "Winner", "Core"] if k in r}
        st.json(_ctx)

        # derive core + seed
        core = str(r.get("Core", "")).zfill(3) if str(r.get("Core","")).strip() != "" else ""
        stream = r.get("Stream", "")
        date_val = pd.to_datetime(r.get("Date", None)) if r.get("Date", None) is not None else None

        if not core or not stream or date_val is None or pd.isna(date_val):
            st.warning("This row is missing Core/Stream/Date fields needed for verification.")
        else:
            seed_digits = _seed_for_stream_asof(_df_all_dbg, stream, date_val)
            st.markdown(f"**Seed digits for this stream as-of {date_val.date()} (previous draw):** `{''.join(sorted(seed_digits))}`")

            # base member probs from training window counts if present; else derive from history window
            base_probs = {}
            for lbl in ["AABC", "ABBC", "ABCC"]:
                for key in [f"MemberTrainCnt_{lbl}", f"TrainCnt_{lbl}"]:
                    if key in r and r.get("MemberTrainN", 0):
                        base_probs[lbl] = float(r.get(key, 0.0)) / float(r.get("MemberTrainN", 1.0))
                        break
            if sum(base_probs.values()) <= 0:
                # derive from history window used in backtest (approx): lookback by training window days setting
                lookback_days = int(st.session_state.get("member_train_window_days", 180))
                d0 = date_val - pd.Timedelta(days=lookback_days)
                hist = _df_all_dbg[(_df_all_dbg["Stream"] == stream) & (_df_all_dbg["Date"] >= d0) & (_df_all_dbg["Date"] <= (date_val - pd.Timedelta(days=1)))]
                # count hits by member label inside this core family
                a, b, c = list(core)
                members = {
                    "AABC": f"{a}{a}{b}{c}",
                    "ABBC": f"{a}{b}{b}{c}",
                ...
                rev = {v: k for k, v in members.items()}
                counts = {k: 0 for k in members}
                for w in hist["Result"].astype(str).str.zfill(4):
                    if w in rev:
                        counts[rev[w]] += 1
                tot = sum(counts.values())
                base_probs = {k: (counts[k] / tot if tot > 0 else 1/3) for k in members}

            st.markdown("**Base member probabilities (before traits):**")
            st.json({k: round(v, 4) for k, v in base_probs.items()})

            # run explanation with the same LUTs currently loaded
            dbg = _explain_member_traits_soft_override(
                core=core,
                seed_digits=seed_digits,
                base_probs=base_probs,
                member_traits_pos_lut=member_traits_pos_lut,
                member_traits_neg_lut=member_traits_neg_lut,
                member_seed_weight=float(st.session_state.get("member_seed_weight", 1.0)),
                auto_mode=bool(enable_auto_traits),
                auto_k=float(auto_shrink_k),
                auto_cap=float(auto_evidence_cap),
                max_total_boost=0.35,
            )

            st.markdown("**Multipliers + adjusted probabilities (after traits):**")
            st.json({
                "mult_raw": {k: round(v, 4) for k, v in dbg["mult_raw"].items()},
                "mult_blend": {k: round(v, 4) for k, v in dbg["mult_blend"].items()},
                "adj_probs": {k: round(v, 4) for k, v in dbg["adj_probs"].items()},
            })

            _df_detail = dbg["detail_df"]
            st.markdown("**Matched trait rows (each row = one trait contribution)**")
            if _df_detail.empty:
                st.info("No member seed-trait rows matched this (core, seed) — adjusted probs will equal base probs.")
            else:
                st.dataframe(_df_detail.sort_values(["MemberLabel","SeedDigit","Dir"]), use_container_width=True)
                st.caption("If this table is empty when you expect matches, the most common causes are: (1) Core formatting mismatch (needs 3-digit), (2) Seed digit mismatch (stream seed not what you expect), or (3) CSV columns not being read into the LUT.")

        # Download: member pick accuracy table
        _dl_member_acc = agg.sort_values(["Top2Rate","Top1Rate","N"], ascending=False).copy()
        _dl_member_acc_csv = _dl_member_acc.to_csv(index=False).encode("utf-8")
        st.download_button(
            "Download member-pick accuracy table (CSV)",
            data=_dl_member_acc_csv,
            file_name=_download_filename(
                "member_pick_accuracy",
                start_dt=start_dt,
                end_dt=end_dt,
                extra=f"basis={member_basis}_cores={len(cores_sel)}",
            ),
            mime="text/csv",
            key="dl_member_pick_accuracy",
        )
    ...
    # -------------------------
    # Safety: avoid NameError if this block is reached in an unexpected execution path
    use_cached_safe = bool(locals().get("use_cached", st.session_state.get("bt_use_cached", False)))
    current_sig_safe = locals().get("current_sig", None)

    if use_cached_safe and (st.session_state.get("bt_last_kpi_sig", None) == current_sig_safe):
        ...
        if _kpi_df is not None and hasattr(_kpi_df, "empty") and (not _kpi_df.empty):
            ...
            st.download_button(
                ...
                file_name=_download_filename("backtest_box_straight_kpis", start_dt=start_dt, end_dt=end_dt),
                mime="text/csv",
        ...
        played_boxes = []
        # Safety: if Streamlit branch/caching bypassed core selection in this run, recover from session_state.
        if "cores_sel" not in locals():
            cores_sel = list(st.session_state.get("bt_cores_sel", []))
        if not cores_sel:
            st.info("Select at least one core to backtest.")
            cores_sel = []
        for _c in cores_sel:
        ...
        # In cached-mode runs, helper vars like all_dates_sorted may not exist.
        if "all_dates_sorted" not in locals():
            try:
                if out is not None and (not getattr(out, "empty", True)) and ("Date" in out.columns):
                    _d = (
                    ...
                    all_dates_sorted = sorted(_d)
                elif "df_all" in locals() and (df_all is not None) and ("Date" in df_all.columns):
                    _d = (
        ...
        # Ensure max_dates is always defined (the input widget can be conditionally rendered).
        try:
            max_dates = int(st.session_state.get("bt_max_dates", 1100) or 1100)
        except Exception:
            max_dates = 1100
        ...
        # Ensure df_all Date is datetime for training (robust to column name variations)
        _df_all_dt = df_all.copy()

        _date_candidates = ["Date", "DrawDate", "draw_date", "date"]
        _date_col = None
        for _c in _date_candidates:
            if _c in _df_all_dt.columns:
                _date_col = _c
                break

        if _date_col is None:
            # Keep the app running, but disable date-driven parts.
            st.error("History file is missing a usable Date column. Expected a column named Date (or similar).")
            _df_all_dt["Date"] = pd.NaT
        else:
            try:
                if _date_col != "Date":
                    _df_all_dt = _df_all_dt.rename(columns={_date_col: "Date"})
                _df_all_dt["Date"] = _parse_date_series(_df_all_dt["Date"])
            except Exception:
                st.error("Unable to parse the Date column from the history file. Date-driven backtests will be disabled.")
                _df_all_dt["Date"] = pd.NaT

        # --- SPEED: pre-sort once and use searchsorted slices (avoids O(N*D) boolean masks) ---
        _df_all_sorted = _df_all_dt.sort_values("Date", kind="mergesort").reset_index(drop=True)
        ...
        if "__BOXKEY__" not in _df_all_sorted.columns:
            try:
                ...
                _df_all_sorted["__BOXKEY__"] = ""
        _all_norm_arr = _df_all_sorted["Date"].dt.normalize().values.astype("datetime64[ns]")


        # Ensure winners_by_date exists for the KPI/backtest loop (robust guard)
        try:
            winners_by_date  # noqa: F401
        except Exception:
            try:
                _tmp_w = _df_all_sorted.dropna(subset=["Date"]).copy()
                _tmp_w["__D__"] = _tmp_w["Date"].dt.normalize()
                winners_by_date = {pd.Timestamp(k).date(): g.drop(columns=["__D__"], errors="ignore")
                                  for k, g in _tmp_w.groupby("__D__", sort=False)}
            except Exception:
                winners_by_date = {}

        for _d in dates_all:
            # winners_by_date keys are python date() objects
            try:
                _d_key = pd.Timestamp(_d).date()
            except Exception:
                _d_key = _d
            g = winners_by_date.get(_d_key, None)
            ...
            for _i, _r in g.iterrows():
                ...
                kpi_rows.append({
                    ...
                    "Stream": stream,
                    "Seed": _seed_for_stream_asof(df_hist, stream, pd.to_datetime(_d)),
                    "Winner": winner,
    ...
    if df_rank.empty:
        ...
        if total_wins == 0:
            ...
            with st.expander("Row hotness table (all rows)", expanded=False):
                ...
                _dl_row_hotness_csv = rc.to_csv(index=False).encode("utf-8")
                st.download_button(
                    "Download row hotness table (CSV)",
                    data=_dl_row_hotness_csv,
                    file_name=_download_filename(
                        "row_hotness",
                        start_dt=start_dt,
                        end_dt=end_dt,
                        extra=f"rankcol={rank_col}_cores={ncores}",
                    ),
                    mime="text/csv",
                    key="dl_row_hotness",
                )
            ...
            _dl_topk_strat = strat_df.copy()
            _dl_topk_strat_csv = _dl_topk_strat.to_csv(index=False).encode("utf-8")
            st.download_button(
                "Download Top‑K row strategies table (CSV)",
                data=_dl_topk_strat_csv,
                file_name=_download_filename(
                    "topk_row_strategies",
                    start_dt=start_dt,
                    end_dt=end_dt,
                    extra=f"rankcol={rank_col}_cores={ncores}_membermult={member_mult}",
                ),
                mime="text/csv",
                key="dl_topk_row_strategies",
            )
            ...
            if manual_rows:
                ...
                with st.expander("Per-core capture for these rows", expanded=False):
                    ...
                    _dl_pc_csv = pc.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Download per-core capture table (CSV)",
                        data=_dl_pc_csv,
                        file_name=_download_filename(
                            "custom_rows_per_core_capture",
                            start_dt=start_dt,
                            end_dt=end_dt,
                            extra=f"rows={'-'.join(map(str, chosen_rows))}",
                        ),
                        mime="text/csv",
                        key="dl_custom_rows_per_core",
                    )
            ...
            if member_track and ("ActualFamilyMember" in out.columns) and (not out.dropna(subset=["ActualFamilyMember"]).empty):
                ...
                _dl_core_top2 = dist[["Core","AABC","ABBC","ABCC","Total","Top1","Top2","Top1Pct"]].copy()
                _dl_core_top2_csv = _dl_core_top2.to_csv(index=False).encode("utf-8")
                st.download_button(
                    "Download core-by-core Top2 recommendation table (CSV)",
                    data=_dl_core_top2_csv,
                    file_name=_download_filename(
                        "core_top2_reco",
                        start_dt=start_dt,
                        end_dt=end_dt,
...
if member_track and (not out.empty):
    ...
    if mc.empty:
        ...
        with st.expander("Download member strategy comparison rows (copy/paste ready)", expanded=False):
            ...
            st.download_button(
                ...
                file_name=_download_filename("member_strategy_comparisons"),
                mime="text/csv",
            )
    ...
    st.download_button(
        ...
        file_name=_download_filename("walkforward_rows"),
        mime="text/csv",
    )
    ...
    try:
        st.caption("By-core predicted % chart")
        _bc = by_core.set_index("Core")[["PredictedPct","Total"]].copy()
        st.bar_chart(_bc)
    except Exception:
        pass
    ...
    try:
        st.caption("By-stream predicted % chart")
        _bs = by_stream.set_index("Stream")[["PredictedPct","Total"]].copy()
        st.bar_chart(_bs)
    except Exception:
        pass
            ...
            if use_straight_booster and (max_straights_per_day > 0) and selected:
                ...
                if sugg_df.empty:
                    ...
                    st.download_button(
                        ...
                        file_name=_download_filename("straight_booster_picks", extra=f"last{int(eval_days)}d"),
                        mime="text/csv",
...
tab_labels = ["Northern Star (v51)", "Northern Lights (Master playlist)", "Core view", "Validation", "Backtest (optional)", "7-Core Execution (Play Today)"]
        ...
        if not (_nl_run_now or _nl_auto):
            ...
            if nl_use_all_cores:
                ...
                if missing:
                    ...
                    st.info("Falling back to selected-cores mode for this run so the app remains usable.")
            ...
            for core in cores_for_cache:
                ...
                for stream in stats_df["Stream"].astype(str).tolist():
                    ...
                    try:
                        ...
                        seed_score = 0.0
                        if st.session_state.get("enable_seed_traits", True) and (seed_traits_pos_lookup or seed_traits_pos_lookup_auto):
                            # AUTO mode uses (lift + support) with shrinkage; otherwise use legacy deltas.
                            if bool(st.session_state.get("auto_traits_mode", False)) and seed_traits_pos_lookup_auto:
                                seed_score, _seed_matches = compute_seed_traits_score_auto(
                                    str(core),
                                    seed,
                                    str(stream),
                                    pos_lookup=seed_traits_pos_lookup_auto,
                                    neg_lookup=seed_traits_neg_lookup_auto,
                                    last5_union_digits_by_stream=_last5_union_by_stream,
                                    cap=float(st.session_state.get("auto_traits_cap", 2.0)),
                                    k=float(st.session_state.get("auto_traits_k", 250.0)),
                                )
                            else:
...
with _t_core:
    ...
    core_ready = True
    if df_all is None or df_all.empty:
        st.info("Upload your all-states history file to populate Core view.")
        core_ready = False
    ...
    if core_ready:
        ...
        st.caption("(Core view will appear automatically after you upload history and select cores.)")
            ...
            try:
                if 'pick_rows' in locals() and pick_rows:
                    ...
                    st.download_button(
                        ...
                        file_name=_download_filename("play_today_box_plan", extra=str(play_date) if play_date else ""),
                        mime="text/csv",
...
with _t_val:
    _df_val = st.session_state.get('_df_all_dt', None)
    if _df_val is None:
        _df_val = df_all
    render_validation(_df_val)
...
with _t_bt:
    ...
    bt_ready = True
    if df_all is None or df_all.empty:
        st.info("Upload your all-states history file to enable Backtest.")
        bt_ready = False
    ...
    if not bt_ready:
        st.caption("(Backtest controls will appear automatically once history is uploaded and cores are selected.)")
    else:
        try:
