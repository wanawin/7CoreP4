...
if "_t_exec" in locals() and _t_exec is not None:
    with _t_exec:
        ...
        try:
            ...
            if cand is None or (isinstance(cand, pd.DataFrame) and cand.empty):
                cand = _load_ns_stream_rank_from_disk(int(window_days))
                if cand is None:
                    # Try alternate windows if the current window wasn't persisted
                    for _w in [180, 365]:
                        if int(_w) == int(window_days):
                            continue
                        cand = _load_ns_stream_rank_from_disk(int(_w))
                        if cand is not None:
                            break

            # If we found a valid table, store it in the canonical session key
