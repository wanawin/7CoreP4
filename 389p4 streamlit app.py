...
def _hard_reset_disk_and_streamlit_caches() -> list[str]:
    """Hard reset: delete disk caches/stores and clear Streamlit caches + relevant session keys.
    Returns a list of human-readable actions performed."""
    actions: list[str] = []
    # 1) Clear Streamlit in-memory caches if available
    try:
        st.cache_data.clear()
    ...
    # 2) Delete known disk cache directories (if present)
    for d in [DISK_CACHE_DIR, BASELINE_STORE_DIR]:
        try:
            if d.exists():
                shutil.rmtree(d, ignore_errors=True)
                actions.append(f"Deleted disk dir: {d.as_posix()}")
        except Exception:
            pass

    # 3) Remove known session_state keys that hold cached artifacts/results
    try:
        keys_to_drop = [k for k in list(st.session_state.keys())
                        if any(tok in str(k).lower() for tok in [
                            "baseline", "pct", "pctmap", "cache", "rankpos", "ns_stream_rank",
                            "backtest", "wf", "walk", "planner"
                        ])]
        for k in keys_to_drop:
            try:
                ...
                pass
        if keys_to_drop:
            actions.append(f"Cleared session_state keys: {len(keys_to_drop)}")
    except Exception:
        pass

    if not actions:
        actions.append("No caches found to clear (or permissions prevented deletion).")
    return actions
    ...
    *,
    ...
    if enable_member_seed_traits_overrides and (member_traits_pos_lut or member_traits_neg_lut):
        try:
            ...
            seed = _seed_for_stream_asof(df_all, stream, test_date) if "stream" in df_all.columns else ""
...
def _build_member_traits_lookup(df: pd.DataFrame) -> Dict[str, Dict[str, Dict[str, Dict[str, float]]]]:
    ...
    member_label is one of AABC/ABBC/ABCC (derived from the Member column's structure).
    """
...
def _apply_member_traits_soft_override(
    ...
    seed_traits = {}
    try:
        seed_traits.update(_seed_traits_for_core_stream(df_all, core, stream, test_date, seed))
    except Exception:
        pass
...
with st.sidebar:
    ...
    with st.expander("Troubleshooting (hard reset)", expanded=False):
        ...
        if st.button("Hard reset caches (disk + memory)", key="hard_reset_btn"):
...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    ...
    if not run:
        ...
        if _cached_out is not None and hasattr(_cached_out, "empty") and (not _cached_out.empty):
            use_cached = True
    ...
    if use_cached:
        ...
        try:
            _df_all_dt["Date"] = pd.to_datetime(_df_all_dt["Date"], errors="coerce")
        except Exception:
            pass
        ...
        for _d in dates_all:
            ...
            hist_train = _df_all_dt[_df_all_dt["Date"].dt.normalize() < _d].copy()
            ...
            for _i, _r in g.iterrows():
                ...
                if kpi_eval_straights and int(kpi_straight_cap) > 0 and (hist_train is not None) and (not hist_train.empty):
                    # Build straight candidates from the exact same helper used in Play Today.
                    # We aggregate across all boxes you would play and then keep the top-probability
                    # straights up to the cap.
                    try:
                        straight_candidates = []
                        for _mb in played_boxes:
                            items, meta = top_straights_for_box_from_history(
                                ...
                                min_stream_samples=int(kpi_min_stream_samples),
                                min_global_samples=max(int(kpi_min_stream_samples) * 2, 50),
                            )
                            if not items:
                                continue
                            ...
                            for straight, p, n, used in items:
                                if isinstance(straight, str) and len(straight) == 4 and straight.isdigit():
                                    straight_candidates.append((float(p), straight, int(n), str(used), cov, _mb))
                        # pick top-probability unique straights
                        ...
                        for p, straight, n, used, cov, mb in straight_candidates:
                            if straight in seen_s:
                                continue
                            selected_straights.append(straight)
                            seen_s.add(straight)
                            if len(selected_straights) >= int(kpi_straight_cap):
                                break
                    except Exception:
                        selected_straights = []
