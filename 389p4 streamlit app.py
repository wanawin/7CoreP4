...
if "_t_exec" in locals() and _t_exec is not None:
    with _t_exec:
        ...
        ranked_lines = None
        try:
            # Prefer the cached Northern Star stream ranking table if available.
            # 1) in-memory session_state (any window)
            # 2) window-specific session_state
            # 3) disk cache (window-specific)
            # 4) disk cache (any supported window) as a fallback
            cand = st.session_state.get("ns_stream_rank_df")

            # window-specific session copy
            if cand is None:
                cand = st.session_state.get(f"ns_stream_rank_df_{window_days}")

            # disk cache (requested window)
            if cand is None:
                cand = _load_ns_stream_rank_from_disk(window_days)

            # disk cache (other windows) — avoids a dead-end if user switches window after building cache
            if cand is None:
                for _alt_w in (180, 365):
                    if _alt_w == window_days:
                        continue
                    cand = _load_ns_stream_rank_from_disk(_alt_w)
                    if cand is not None:
                        break

            if isinstance(cand, pd.DataFrame) and not cand.empty:
                # normalize columns
                if "Stream" not in cand.columns and "stream" in cand.columns:
                    cand = cand.rename(columns={"stream": "Stream"})
                if "Core" not in cand.columns and "core" in cand.columns:
                    cand = cand.rename(columns={"core": "Core"})

                # keep a canonical copy for later tabs
                st.session_state["ns_stream_rank_df"] = cand
                # also keep window-specific if it matches current window
                if window_days in (180, 365):
                    st.session_state[f"ns_stream_rank_df_{window_days}"] = cand

                # Filter to the 7 cores and build ranked lines
                cand2 = cand.copy()
                if "Core" in cand2.columns:
                    cand2["Core"] = cand2["Core"].astype(str).str.zfill(3)

                # keep only 7-core set
                cand2 = cand2[cand2["Core"].isin(execution_cores)].copy()

                # ensure we have a score column
                score_col = None
                for c in ("Score", "score", "BaseScore", "basescore"):
                ...
                cand2[score_col] = pd.to_numeric(cand2[score_col], errors="coerce").fillna(0.0)

                # Build ranked lines: highest score first
                cand2 = cand2.sort_values(score_col, ascending=False)
                ranked_lines = [(r["Stream"], r["Core"], float(r[score_col])) for _, r in cand2.iterrows() if "Stream" in r and "Core" in r]
        except Exception:
            ranked_lines = None

        if ranked_lines is not None and not ranked_lines.empty:
            show_cols = [c for c in ["Stream", "core", "BaseScore", "RankPos"] if c in ranked_lines.columns]
        ...
        if ranked_lines is not None and not ranked_lines.empty and play_date is not None:
            ...
            if pick_rows:
                ...
                st.dataframe(pd.DataFrame(pick_rows).head(40), use_container_width=True, hide_index=True)
