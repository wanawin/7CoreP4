...
# Resolve a usable date column
_date_col = None
for _c in ["Date", "date", "DrawDate", "draw_date", "Draw Date", "DATE"]:
    if _c in _df_all_dt.columns:
        _date_col = _c
        break
if _date_col is None:
    # try any column that contains "date"
    for _c in _df_all_dt.columns:
        if "date" in str(_c).lower():
            _date_col = _c
            break

if _date_col is not None and _date_col != "Date":
    try:
...
if "Date" in _df_all_dt.columns:
    try:
        _df_all_dt["Date"] = _parse_date_series(_df_all_dt["Date"])
    except Exception:
        pass
else:
    # Hard-stop later code paths that assume Date exists
