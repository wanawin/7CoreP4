...
# --- BUILD STAMP (auto) ---
APP_BUILD_ID = 'v123_BACKTEST_TRACE_UI_LABEL_20260217T222938Z'
APP_BUILD_BASE = 'v122_BACKTEST_PANDAS_GET_MONKEYPATCH'
APP_BUILD_HASH = '2b04c54119'
# --------------------------
...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    def _row_get(_row, _key, _default=""):
        """Safe column access for Series / dict / itertuples rows."""
        if _row is None:
            return _default
        if hasattr(_row, "get"):
            try:
                return _row.get(_key, _default)
            except Exception:
                pass
        # itertuples() rows expose columns as attributes; pandas may sanitize names
        for _cand in (_key, _key.replace(" ", "_"), re.sub(r"\W+", "_", _key)):
            if hasattr(_row, _cand):
                _v = getattr(_row, _cand)
    ...
    if not use_cached:
        ...
        _date_vals = pd.to_datetime(_df_sorted['Date'], errors='coerce').to_numpy(dtype='datetime64[ns]')
        ...
        for test_date in dates_to_test:
            ...
            for wr in day_winners.itertuples(index=False):
                stream = str(getattr(wr, "Stream", "")).strip()
