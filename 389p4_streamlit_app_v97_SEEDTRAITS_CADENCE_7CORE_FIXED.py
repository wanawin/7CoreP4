...
if "_t_exec" in locals() and _t_exec is not None:
    with _t_exec:
        ...
        if ranked_lines is not None and len(ranked_lines) > 0:
            ...
            if cadence_table_df is not None and not cadence_table_df.empty:
                try:
                    ...
                    def _ensure_cadence_cols(df: pd.DataFrame) -> pd.DataFrame:
                        df = df.copy()
                        if "DaysSinceLastHit" not in df.columns and "days_since_last_hit" in df.columns:
                            df["DaysSinceLastHit"] = df["days_since_last_hit"]
                        if "HitsPerWeek" not in df.columns and "hits_last180" in df.columns:
                            df["HitsPerWeek"] = (pd.to_numeric(df["hits_last180"], errors="coerce").fillna(0) * 7.0) / 180.0
                        if "CadenceScore" not in df.columns:
                            hits180 = pd.to_numeric(df["hits_last180"], errors="coerce").fillna(0) if "hits_last180" in df.columns else 0
                            dslh = pd.to_numeric(df["DaysSinceLastHit"], errors="coerce").fillna(9999) if "DaysSinceLastHit" in df.columns else 9999
                            df["CadenceScore"] = ((hits180 > 0) & (dslh <= 180)).astype(int)
                        return df
            ...
            # Compute SeedTraitsScore only when inputs exist; otherwise keep as 0 (soft boost).
            if enable_seed_traits_boost and ("seedtraits_pos_lookup" in globals()) and ("seedtraits_neg_lookup" in globals()):
                try:
                    ranked_lines["SeedTraitsScore"] = ranked_lines.apply(
                        lambda r: compute_seed_traits_score(
                            core=str(r[core_col]).zfill(3),
                            seed=str(r["Seed"]).zfill(4),
                            pos_lookup=seedtraits_pos_lookup,
                            neg_lookup=seedtraits_neg_lookup,
                        ),
                        axis=1,
                    )
                except Exception:
                ...
                ranked_lines["SeedTraitsScore"] = ranked_lines.get("SeedTraitsScore", 0.0).fillna(0.0)
            seed = ranked_lines["SeedTraitsScore"]
            cad  = ranked_lines.get("CadenceScore", 0.0)

            ranked_lines["BaseTerm"] = base_w * base
            ranked_lines["DueTerm"]  = due_w * due
            ranked_lines["PosTerm"]  = pos_w * pos
            ranked_lines["SeedTerm"] = seed_traits_weight * seed
            ranked_lines["CadTerm"]  = cadence_weight * pd.to_numeric(cad, errors="coerce").fillna(0.0)


            # 5) RecomputedScore for debug display (does NOT alter ranking engine output)
            ranked_lines["RecomputedScore"] = (
                ranked_lines["BaseTerm"] + ranked_lines["DueTerm"] + ranked_lines["PosTerm"] + ranked_lines["SeedTerm"] + ranked_lines["CadTerm"]
            ).astype(float)

            # If UniversalScore is missing or all zeros in this table, show the recomputed value (display only)
            if "UniversalScore" not in ranked_lines.columns:
                ranked_lines["UniversalScore"] = ranked_lines["RecomputedScore"]
            else:
                _u = pd.to_numeric(ranked_lines["UniversalScore"], errors="coerce").fillna(0.0)
                if _u.eq(0.0).all():
                    ranked_lines["UniversalScore"] = ranked_lines["RecomputedScore"]
                else:
                    ranked_lines["UniversalScore"] = _u
